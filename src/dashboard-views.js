...
export function renderOverview({ summary, rows, filter, sort, order, totalKommuner, q = '', actionQueue = [], waiting = [], vacationActive = false, heartbeat = null, partial = false, escalationCount = 0 }) {
  ...
  // The funnel: one clickable chevron per pipeline stage (same collapse rule
  // as the /pipeline board — furthest conversation wins, stoppat never wins).
  // Clicking a step filters the table to kommuner standing at that stage;
